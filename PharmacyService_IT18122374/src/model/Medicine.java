...
public class Medicine {
	...
	private Connection connect() {
		...
		try {
			/*
			 * Class.forName("com.mysql.jdbc.Driver"); //Provide the correct details:
			 * DBServer/DBName, username, password con =
	...
	public String insertMedicine(String code, String name, String price, String desc) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement(
					" CREATE TABLE oshadi (itemID int NOT NULL AUTO_INCREMENT, itemCode varchar(50),itemName varchar(50),itemPrice varchar(50),itemDesc varchar(50), primary key (itemID)); ");
			// ps.execute();

			String query = " insert into oshadi(`itemID`,`itemCode`,`itemName`,`itemPrice`,`itemDesc`)"
					+ " values (?, ?, ?, ?, ?)";
		...
		try {
			...
			while (rs.next()) {
				...
				output += "<td><input name=\"btnUpdate\" type=\"button\"value=\"Update\" class=\"btnUpdate btn-secondary\"></td>"
						+ "<td><form method=\"post\" action=\"medicines.jsp\">"
